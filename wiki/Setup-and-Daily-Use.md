...
Everything after [flashing](Flashing.md): getting the device onto your WiFi, then living with it.
...
1. The device screen shows a WiFi network named `UsageStick-XXXX` along with its password. Join that network from your phone or laptop.
...
The device encrypts the token with your PIN, stores it, and connects to your WiFi. See [Security](Security.md) for what that encryption actually does.

> On the [ESP32-C3-OLED](ESP32-C3-OLED.md) the screen is too small for the full random password, so it shows a simpler 8-digit one instead.
...
On ✨ **Dust** (v3) boards there's a second way: logging into the **[web panel](Web-Panel.md)** from any browser on your network unlocks the screen too — no button-cycling. Reach it at `usage-stick.local` (or your device-name slug); by design the locked screen doesn't display the address — it appears in the dashboard header only after unlock. The panel is also where settings, token rotation and WiFi changes live after setup.
...
Boards without two buttons map this differently — one button by press duration, or touch zones on the touch panels. Check your [board's guide](Supported-Boards.md):

- [LilyGo T8 ESP32-S2](LilyGo-T8-ESP32-S2.md) — short tap = A, long press = B
- [CrowPanel Advance 3.5"](CrowPanel-Advance-35.md) — tap left half = A, tap right half = B
- [T-Display S3 AMOLED](LilyGo-T-Display-S3-AMOLED.md) — physical button = A, tap the screen = B
- [ESP32-C3-OLED](ESP32-C3-OLED.md) — buttons you wire yourself on GPIO 3 and GPIO 7

## Factory reset

Holding **A+B** while the device boots wipes every stored credential and returns it to setup mode.

On ✨ Dust boards you rarely need it: WiFi changes and [token rotation](Web-Panel.md#rotating-the-token) both live in the web panel, an unreachable network drops the device into a recovery portal by itself, and the panel's *Danger zone* can trigger the same full wipe from the browser.
