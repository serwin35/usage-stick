...
Firmware releases carry names. Which one your board runs is listed in [Supported boards](Supported-Boards.md).
...
| v3 | ✨ **Dust** | Everything in Mango, plus the [web panel](Web-Panel.md), screen modes (carousel · 7-day chart · Anthropic news · desk clock), timezone, mDNS, WiFi recovery, and token rotation without a reset |

Boards still on **Clarity** keep the original minimal dashboard until they're migrated to their tier. **Dust** runs on the [T-Display S3](LilyGo-T-Display-S3.md) and the [M5StickC Plus](M5StickC-Plus.md); the rest follow as they migrate.
...
| ---- | ----- | ---------- | --------------- | -------------- | ------ |
| **XS** | tiny OLED | ≤ 128×64 | [ESP32-C3-OLED](ESP32-C3-OLED.md) | — | ⏳ Pending |
| **S** | small LCD | ~240×135 | [M5StickC Plus](M5StickC-Plus.md) | One overall-health Clawd + a 2×2 `NAME UP/DOWN` text grid | ✅ |
| **L** | large LCD | ~320×170 | [LilyGo T-Display S3](LilyGo-T-Display-S3.md) | A row of four labelled Clawds (one per model), each blinking when healthy | ✅ |
| **XL** | big / touch | ≥ 480×320 | [CrowPanel 3.5"](CrowPanel-Advance-35.md), [S3 AMOLED](LilyGo-T-Display-S3-AMOLED.md) | — | ⏳ Pending |
...
- **The [web panel](Web-Panel.md)** — every setting, token rotation, and WiFi changes from a browser on your LAN; logging in there also unlocks the device
- **Timezone** — a GMT± setting that anchors the clock screen and the chart's day markers
- **Header address** — the orange header alternates between the device name and its `http://` address; `usage-stick.local` works too (mDNS)
